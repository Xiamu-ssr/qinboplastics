# 秦帛新材料官网

> 🌐 [www.qinboplastics.com](http://www.qinboplastics.com)

上海秦帛新材料 / 海口秦帛新材料 — 企业官方网站。

部署在阿里云 OSS 上，纯静态站点。

---

## 公司简介

秦帛新材料是一家专注于塑料及新材料领域的企业，旗下有两家主体公司：

- **上海秦帛新材料** — 总部/运营中心
- **海口秦帛新材料** — 生产/区域运营

主营业务：塑料原料及改性材料的销售与服务。

---

## 同行大厂参考

| 公司 | 官网 | 国家 | 主营 | 技术栈 | 网站类型 |
|------|------|------|------|--------|----------|
| **杜邦 DuPont** | [dupont.com](https://www.dupont.com) | 🇺🇸 美国 | 特种材料、电子、水处理 | Adobe AEM + React + Next.js | ❌ 动态（企业级 CMS） |
| **巴斯夫 BASF** | [basf.com](https://www.basf.com) | 🇩🇪 德国 | 化工全品类、工程塑料 | Adobe AEM + React/Vue/Angular + Next.js | ❌ 动态（企业级 CMS） |
| **科思创 Covestro** | [covestro.com](https://www.covestro.com) | 🇩🇪 德国 | 聚碳酸酯、聚氨酯 | Sitecore (.NET) + Next.js | ❌ 动态（企业级 CMS） |
| **SABIC** | [sabic.com](https://www.sabic.com) | 🇸🇦 沙特 | 石化、工程塑料 | 自研后端 + CDN | ❌ 动态（有会话管理） |
| **LG 化学** | [lgchem.com](https://www.lgchem.com) | 🇰🇷 韩国 | 石化、电池材料 | Spring (Java) + Next.js + jQuery | ❌ 动态（Java 后端） |
| **金发科技 Kingfa** | [kingfa.com](https://www.kingfa.com) | 🇨🇳 中国 | 改性塑料、降解塑料 | 传统服务端 + jQuery + CDN (Tengine) | ❌ 动态（传统 CMS） |

### 结论

**所有大厂官网都是动态网站**，主要原因：

1. **内容管理系统 (CMS)** — 产品成百上千，需要后台管理产品信息、新闻、文档
2. **多语言** — 国际化企业需要多语言内容切换
3. **用户系统** — 会员登录、样品申请、在线询价、技术文档下载等
4. **搜索/筛选** — 产品目录需要动态搜索和筛选
5. **数据分析** — 用户行为跟踪、A/B 测试

**但秦帛不需要这些。** 对于中小型材料贸易企业，静态网站完全够用。

---

## 秦帛官网设计方案

### 为什么静态站就够了？

秦帛目前的需求是：让客户能找到我们、知道我们卖什么、怎么联系我们。这不需要 CMS 后台、不需要用户系统、不需要在线下单。一个精心设计的静态站就是最优解：

- ✅ 加载快（OSS + CDN 直出）
- ✅ 零运维成本（没有服务器、没有数据库）
- ✅ 安全（纯静态无攻击面）
- ✅ 便宜（OSS 存储 + 流量费极低）

### 技术选型

```
HTML + CSS + JavaScript（原生或轻量框架）
├── 无构建工具依赖，直接部署到 OSS
├── 响应式设计（手机/平板/电脑）
└── SEO 友好（纯 HTML，搜索引擎直接抓取）
```

**不用 React/Vue/Next.js** — 对于 5-6 页的企业站，框架是过度工程化。纯 HTML/CSS/JS 更轻量、维护更简单、SEO 更好。

### 页面规划

```
www.qinboplastics.com/
├── index.html          # 首页
├── about.html          # 关于我们
├── products.html       # 产品中心
├── products/           # 产品详情页（按品类）
│   ├── category-1.html
│   └── category-2.html
├── news.html           # 新闻动态（可选）
└── contact.html        # 联系我们
```

### 首页 (index.html)

```
┌──────────────────────────────────────────┐
│  LOGO    首页  关于我们  产品中心  联系我们  │  ← 固定导航栏
├──────────────────────────────────────────┤
│                                          │
│           全屏 Banner / 轮播             │  ← 工厂/产品大图
│     "专业塑料原材料供应商"                 │
│         [ 了解更多 ]  [ 联系我们 ]         │
│                                          │
├──────────────────────────────────────────┤
│                                          │
│  🏢 关于秦帛          📦 核心产品          │  ← 3-4 个亮点卡片
│  🌍 服务网络          ✅ 品质保证          │
│                                          │
├──────────────────────────────────────────┤
│                                          │
│         产品分类展示（带图）               │  ← 3-6 个主要品类
│    [ 通用塑料 ] [ 工程塑料 ] [ ... ]      │
│                                          │
├──────────────────────────────────────────┤
│                                          │
│     公司数据 / 里程碑                     │  ← 成立年份、服务客户数等
│     "XX 年行业经验 / XX+ 合作客户"        │
│                                          │
├──────────────────────────────────────────┤
│                                          │
│  上海秦帛新材料          海口秦帛新材料     │  ← 页脚：两个主体信息
│  地址 / 电话 / 邮箱     地址 / 电话 / 邮箱 │
│                                          │
│        © 2024 秦帛新材料 版权所有          │
│              ICP备案号                    │
└──────────────────────────────────────────┘
```

### 关于我们 (about.html)

- 公司介绍（上海 + 海口两个主体）
- 发展历程 / 时间线
- 企业优势（供应链、品质、服务）
- 合作品牌 / 资质证书

### 产品中心 (products.html)

- 产品分类网格展示
- 点击进入品类详情页
- 每个品类：产品列表 + 规格参数 + 应用场景

### 联系我们 (contact.html)

- 两个公司的完整联系信息
- 嵌入地图（高德/百度地图 iframe）
- 在线留言表单（可用第三方表单服务，如腾讯问卷/金数据，或直接留邮箱和电话）

### 设计风格

参考同行但更简洁：
- **配色**：蓝色/白色为主（化工行业常用，传达专业、可信赖）
- **字体**：思源黑体/系统字体（无需加载外部字体，加速渲染）
- **图片**：高质量产品图 + 工厂/仓库实景照
- **动效**：适度（滚动渐入、数字动画），不花哨

---

## 站点结构（已实现）

```
qinboplastics/
├── index.html        # 首页（hero / 关于 / 产品 / 贸易 / 数字墙 / 优势 / CTA）
├── about.html        # 关于我们（简介 / 双主体 / 优势）
├── products.html     # 产品中心（6大品类详情，锚点跳转）
├── trade.html        # 国内外贸易（国内分销 / 国际进出口 / 服务流程）
├── contact.html      # 联系我们（双主体信息 / 留言表单 / 地图占位）
└── assets/
    ├── css/style.css # 全站样式（响应式，LG化学风格）
    ├── js/main.js    # 导航 / 滚动渐入 / 数字动画
    └── img/
        ├── photos/   # 真实照片（临时占位，待替换）
        └── *.svg     # 品类抽象配图
```

纯静态，无构建步骤，无框架依赖 —— 直接上传 OSS 即可运行。

## 本地预览

```bash
cd qinboplastics
python3 -m http.server 8765
# 浏览器打开 http://localhost:8765
```

## 部署到阿里云 OSS

1. **开启静态网站托管**：OSS Bucket → 基础设置 → 静态页面 → 默认首页填 `index.html`
2. **上传文件**：将本目录下所有文件（保持目录结构）上传到 Bucket 根目录
3. **绑定域名**：传输管理 → 域名管理 → 绑定 `www.qinboplastics.com`（需已备案）
4. **配置 HTTPS**：上传 SSL 证书或用阿里云免费证书
5. **挂 CDN（强烈建议）**：加速国内访问 + 降低流量费

```bash
# 也可用 ossutil 命令行批量上传：
# ossutil cp -r ./ oss://你的bucket名/ --update
```

### 占位资源说明
- `assets/img/photos/` 内为临时免版权图（Unsplash），**上线前请替换为秦帛真实的工厂/仓库/产品/团队实拍图**
- `assets/img/*.svg` 为品类抽象配图，可保留或替换为产品实拍
- 所有 `____________（待补充）` 处需填入真实信息（地址/电话/邮箱/ICP备案号）

---

## 待办（需秦帛提供）

- [ ] **真实图片**：工厂 / 仓库 / 产品 / 团队实拍（决定成品质感的关键）
- [ ] **公司 logo**（当前用 "QB" 文字标识临时替代）
- [ ] 产品分类与具体品名核对（当前：通用/工程/改性/进口/橡胶/环保）
- [ ] 数字墙真实数据（合作客户数、品类数等，当前为占位）
- [ ] 两个主体的联系方式（地址 / 电话 / 邮箱 / 联系人）
- [ ] ICP 备案号
- [ ] 公司简介文案审核（当前为基于业务画像撰写的初稿）
- [ ] 域名 SSL 证书 / CDN 配置
- [ ] 是否需要英文版（外贸客户）
